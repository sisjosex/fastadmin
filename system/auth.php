...
class Auth extends Module {
    ...
    function authenticate( $user, $callback = "" ) {
        ...
        $result = $this->get_where( "user.status='active' AND user.email='{$user['email']}' AND user.password='{$user['password']}' AND ( user.role='sadmin' OR user.role='admin' )" );
        ...
        if(!$result) {
            ...
            $response = lang('Invalid user or password');

            $result = $this->get_where( "user.status='inactive' AND user.email='{$user['email']}' AND user.password='{$user['password']}' AND ( user.role='sadmin' OR user.role='admin' )" );

            if($result) {
                $response = lang('Your account is inactive');
            }
        }
