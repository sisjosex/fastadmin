...
/*if(isset( $_GET['lang'] ) ) {
    require_once(BASE_PATH . "language/{$_GET['lang']}.php");
} else {
    require_once(BASE_PATH . 'language/es.php');
}*/
...
class Model {
    ...
    */
    function __construct($table = '', $id = '', $field = 'name') {
        ...
        $this->phisical_table = $this->name = $table;
        $this->table_id = $id;
        $this->field_id = $id;
     ...
     *
     */
    function update($values, $where = array(), $table='', $addslashes = true) {
        ...
        if(!empty($where)) {
            ...
            $sql .= ' WHERE ' . implode(' AND ', $where_values);
        }
    ...
    function delete($id, $table='') {
        ...
        $this->delete_where(array($this->field_id => $id), $table);
    }
    ...
    function get_where_only($search, $table='') {
    ...
    function get($id, $table='') {
        ...
        $sql = "SELECT * FROM `".($table ? $table : $this->phisical_table)."` WHERE {$this->field_id}='$id'";
